...
//    @Fuzz
//    public void testParseInto_monthDay_feb29_newYork_startOfYear(@InRange(minInt=1900, maxInt=2020) int year) {
//        try {
//            DateTimeFormatter f = DateTimeFormat.forPattern("M d").withLocale(Locale.UK);
//            final MutableDateTime expected = new MutableDateTime(year, 2, 29, 0, 0, 0, 0, NEWYORK);
//            final MutableDateTime result = new MutableDateTime(year, 1, 1, 0, 0, 0, 0, NEWYORK);
//            f.parseInto(result, "2 29", 0);
//            Log.logOutIf(false,() -> new Long[] { result.getMillis() }, new Long[]{expected.getMillis()});
//        }catch (Exception e){
//            Log.logOutIf(!((year%4==0&&year%100!=0)||(year%4==0&&year%100==0&&year%400==0)),()->new String[]{"Unexpected Exception"}, new Long[]{new MutableDateTime(year, 2, 29, 0, 0, 0, 0, NEWYORK).getMillis()});
////            if(Log.runBuggyVersion&&(year%4==0)){
////                if(year%100==0)
////                    if(year%400==0)
////                        logger.logOut(new MutableDateTime(year, 2, 29, 0, 0, 0, 0, NEWYORK).getMillis());
////                    else
////                        logger.logOut("Exception");
////                else
////                    logger.logOut(new MutableDateTime(year, 2, 29, 0, 0, 0, 0, NEWYORK).getMillis());
////            }else
////                logger.logOut("Exception");
...
//    }
    ...
    @Fuzz
    public void testParseInto_monthDay_feb29_tokyo_endOfYear(@InRange(minInt=1999, maxInt=2009) int year,
                                                             @InRange(minInt=1, maxInt=12, isFixed=true) int month,
                                                             @InRange(minInt=1, maxInt=12, isFixed=true) int test_month,
                                                             @InRange(minInt=1, maxInt=29, isFixed=true) int test_day,
                                                             ...
                                                             @InRange(minInt=1, maxInt=5) int z){
        try {
            DateTimeZone d = null;
            switch(z) {
                case 1:
            ...
            final MutableDateTime expected = new MutableDateTime(year, test_month, test_day, hour, minute, second, msecond, d);
            String str = test_month + " "+test_day;
            int ret1 = f.parseInto(result, str, 0);
            Log.logOutIf(str.length() == ret1, () -> new Integer[] { ret1 });
            Log.logOutIf(result.equals(expected), () -> new String[] { expected.toString() });
        }catch (Exception e){
            Log.ignoreOut();
